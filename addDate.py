...
def add_header(input_pdf, output_pdf, header_text):
    ...
    for page in reader.pages:
        ...
        can.drawString(150, height - 30, header_text)
        can.save()
...
label = tk.Label(root, text="ЄДИНА КВИТАНЦІЯ ПРО ОПЛАТУ КОМУНАЛЬНИХ ПОСЛУГ", fg="BLACK", font=courier_18)
...
lf13 = ttk.Frame(root, borderwidth=10, relief=SUNKEN)
lf13.config(width=850, height=600)
lf13.grid_propagate(False)

lf12 = ttk.Frame(lf13, borderwidth=10, relief=SUNKEN)
lf12.config(width=width_frame, height=130)
lf12.grid_propagate(False)
...
lf12.grid(column=0, row=0, padx=20, pady=10, sticky=W)
...
lf13.grid(column=1, row=1, ipadx=6, ipady=6, padx=20, pady=20)
